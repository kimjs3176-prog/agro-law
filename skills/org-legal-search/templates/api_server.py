...
if _origins_env:
    ...
    _cors_origins = [
        re.compile(r"^https://your-app[\w.-]*\.vercel\.app$"),  # ← 배포 도메인에 맞게 수정(또는 ALLOWED_ORIGINS 환경변수)
        re.compile(r"^http://localhost(:\d+)?$"),
        re.compile(r"^http://127\.0\.0\.1(:\d+)?$"),
    ]
CORS(app, origins=_cors_origins)

OC   = os.environ.get("LAW_OC", "")   # 법제처 OpenAPI OC 키(필수) — 환경변수 LAW_OC 로 설정
...
DOMAIN_LAW_MAP = {
    # [기관 설정] 시나리오 텍스트에서 이 키워드가 감지되면 해당 법령만 우선 탐색합니다.
    # 우리 기관 업무 도메인에 맞는 키워드→법령 매핑으로 교체하세요.
    "개인정보": ["개인정보 보호법"],
    "정보공개": ["공공기관의 정보공개에 관한 법률"],
    "계약":    ["국가를 당사자로 하는 계약에 관한 법률"],
    "저작권":  ["저작권법"],
    "청탁":    ["부정청탁 및 금품등 수수의 금지에 관한 법률"],
}


# [기관 설정] 조문 검색 대상 핵심 법령 — 우리 기관 소관/관련 법령으로 교체하세요.
CANDIDATE_LAWS = [
    "개인정보 보호법",
    "공공기관의 정보공개에 관한 법률",
...
# 항상 탐색할 핵심 행정규칙(운영규정·훈령·예규) — 법령명 검색에 안 걸리는 규정을 고정 포함.
# [기관 설정] 우리 기관 소관 훈령/예규가 있으면 정식 명칭으로 넣으세요(없으면 빈 목록).
CANDIDATE_ADMRUL = [
]
...
def scenario_search():
    ...
    AVAILABLE_LAWS = (
        "개인정보 보호법, 공공기관의 정보공개에 관한 법률, "
        "국가를 당사자로 하는 계약에 관한 법률, 근로기준법, 행정절차법, "
        "저작권법, 부정청탁 및 금품등 수수의 금지에 관한 법률"
    )
...
SAGYU_MCP_URL = os.environ.get("SAGYU_MCP_URL", "").strip()   # 내규 검색 MCP 서버(Streamable HTTP) 엔드포인트
...
class _McpClient:
    ...
    def initialize(self):
        res = self._post("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "org-legal-search", "version": "1.0"},
        })
        try:
...
_RULE_MARKER_RE = re.compile(
    r"^\s*(?:\[\d+\]|\d+\s*[.)]|[가-힣]\s*[.)]|[①-⑳㉠-㉭]|"
    r"[-–—*○◯●■□▪▶▷◆◇◦·•※☆★✓❍→▶️])\s*")
# 정관은 기관당 1개 — 옛 명칭/약칭 등 본문 인용에서 딸려오는 정관명은 제외.
# [기관 설정] 기관 통폐합·개명 이력이 있으면 옛 명칭 토큰을 넣어 오탐을 줄이세요. 예) ("옛기관명","약칭")
_STALE_ORG_TOKENS = ()
